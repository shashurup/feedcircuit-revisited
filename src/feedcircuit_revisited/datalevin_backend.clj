(ns feedcircuit-revisited.datalevin-backend
...
(def schema (update-vals (->> s/schema
                              (map #(vector (:db/ident %) %))
                              (into {}))
                         #(dissoc % :db/ident)))
(def content-dir)
...
(defn find-max-num []
  (->> (d/rseek-datoms (d/db conn) :ave :item/num)
       first
       d/datom-v))

(defonce cur-item-num (atom 0))

(def feed-attrs (conj (map :db/ident s/feed-schema) :db/id))
...
(def item-attrs (conj (map :db/ident s/item-schema) :db/id))
...
(defn append-items! [url items]
  (let [feed (get-feed-attr url :db/id)
    ...
    (let [tempids (:tempids (d/transact! conn txdata))]
...
(defn add-content! [uid content]
  (d/transact! conn [{:db/id (u/as-int uid)
...
(defn get-items
  ...
  ([feed start reverse]
   (let [db (d/db conn)
         ...
         start (or start (if reverse @cur-item-num 0))
         seek (if reverse d/rseek-datoms d/seek-datoms)]
     (->> (seek db :ave :item/feed+num [feed-id start])
          (map #(d/pull db (conj item-list-attrs :item/feed) (d/datom-e %)))
  ...
  (let [{old-styles :user/styles
    ...
    (let [[updates deletes] (diff-sources new-sources old-sources)
          ...
          styles-tx-data (prepare-styles-tx old-styles
                                            styles
                                            user-id)]
      (d/transact! conn (concat src-tx-data styles-tx-data)))))
...
(defn init-impl! []
  (def content-dir (conf/param :datalevin :content-dir))
  (when (not-empty content-dir)
    (fs/mkdirs content-dir))
  (def conn (d/get-conn (conf/param :datalevin :dir) schema))
